...
- **커버 이미지:** `world_json.cover_image_url`(HTTPS만)·간편 에디터「커버 이미지 URL」→ 공개 상세 상단 히어로. Claude는 이미지 생성 미지원 — DALL·E·Replicate 등 **별도 이미지 API**로 만들고 URL만 저장하는 흐름이 일반적. **자동 생성·NPC 아바타 로드맵:** [`docs/WORLD_VISUAL_AI_ROADMAP.md`](docs/WORLD_VISUAL_AI_ROADMAP.md).
